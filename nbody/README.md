...
- `perf_report_nbody.txt` — call-graph text report generated from `perf.data`.
...
- `perf_report_nbody_swevents.txt` — call-graph report sampled on software
...
lightweight — only the derived `perf_report_nbody_swevents.txt` is kept.
...
perf report --stdio -i perf_swevents.data > perf_report_nbody_swevents.txt
```
