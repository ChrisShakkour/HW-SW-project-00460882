#!/usr/bin/env bash
# script_nbody.sh — Section 10 deliverable for the nbody benchmark.
#
# Runs the full pipeline documented in README.md (Sections 1, 4, 5, 7, 8)
# end to end: environment setup, baseline perf profiling + flame graph, all
# four optimization attempts' runs, and a before/after comparison table.
#
...
# flamegraph_*.svg / perf_stat_*.txt in original/ and each optimized_*/
# directory, overwriting whatever is currently there with a fresh run
# (numbers will vary slightly run to run — see COMPARISON.md and
# OPTIMIZATIONS.md for the committed reference numbers this repo's
...
OPTIMIZED_VARIANTS=(
  optimized_numpy_experiment
  optimized_sqrt_experiment
  optimized_indexflat_experiment
  optimized_unroll_experiment
)
...
echo
echo "############################################"
echo "# [3/4] Post-optimization runs: all four attempts"
echo "############################################"
for variant in "${OPTIMIZED_VARIANTS[@]}"; do
  profile_variant "$SCRIPT_DIR/$variant" "$variant"
done
...
echo "Original (baseline): ${orig_s} s"
echo
printf "%-32s %12s %10s %8s\n" "Variant" "Mean (s)" "Change" "Result"
for variant in "${OPTIMIZED_VARIANTS[@]}"; do
  opt_s=$(parse_seconds "$SCRIPT_DIR/$variant/run_output_${variant}.txt")
  python3 - "$orig_s" "$opt_s" "$variant" <<'PYEOF'
import sys
orig, opt, name = float(sys.argv[1]), float(sys.argv[2]), sys.argv[3]
pct = (orig - opt) / orig * 100
result = "FASTER" if pct > 0 else "slower"
print(f"{name:<32} {opt:>12.4f} {pct:>9.1f}% {result:>8}")
PYEOF
done
echo
